(() => {
  ...
  const GOLDEN_BUTTERFLY_HELPER_DODGES = 3;
  const GOLDEN_BUTTERFLY_HELPER_DODGE_COOLDOWN_MS = 520;
  const COIN_PACKS = [
  ...
  const LEVEL_RULE_BOOK = (() => {
    ...
    const makeGames = ({
      ...
      requirementExtras = {},
      prizeExtras = {},
      prizeMainValues = BASE_GAME_TARGETS
    }) => BASE_GAME_TARGETS.map((target, index) => {
      const requirement = {
        ...requirementExtras,
        [requirementMainKey]: target
      };
      const prize = {
        ...prizeExtras,
        [prizeMainKey]: prizeMainValues[index] ?? target
      };
      return {
        game: index + 1,
        requirement,
  ...
  function getStageFrogColor(level = state.level, frogIndex = 0) {
    const safeLevel = Math.max(1, Math.floor(level));
    if (safeLevel <= 1) {
      return "#52d768"; // Only Level 1 stays classic green.
    }

    // From Level 2+ color rotates by level so progression always feels visible.
    const levelPalette = [
      "#3fb6ff", // sky blue
      "#a55bff", // purple
      "#ffd54f", // gold
      "#ff8f5e", // coral
      "#8d9fb0", // elder gray-blue
      "#61d4ff", // aqua
      "#b47dff", // lilac
      "#ff7ab8", // pink
      "#ffa84a", // orange
    ...
    const startIndex = (safeLevel - 2) % levelPalette.length;
    const index = (startIndex + Math.max(0, frogIndex)) % levelPalette.length;
  ...
  function getFoodHealthBand(daysLeft) {
    if (daysLeft >= 3) return "safe";
    if (daysLeft >= 1) return "low";
    return "critical";
  }

  function updateHomeHealthMeter(daysLeft = getFoodCoverageDays(), healthBand = getFoodHealthBand(daysLeft)) {
    if (!ui.homeHealthFill || !ui.homeHealthTrack) return;

    const meterMaxDays = 5;
    const meterPercent = clamp((daysLeft / meterMaxDays) * 100, 0, 100);
    ...
    if (state.language === "es") {
      if (healthBand === "safe") {
        statusText = `Salud alta • comida para ~${daysShort} dias.`;
      } else if (healthBand === "low") {
        statusText = `Salud media • alimenta pronto (~${daysShort} dias).`;
      } else {
        statusText = "Salud critica • alimenta a Sapito ahora.";
      }
    } else if (healthBand === "safe") {
      statusText = `High health • food for ~${daysShort} days.`;
    } else if (healthBand === "low") {
      statusText = `Medium health • feed soon (~${daysShort} days).`;
    } else {
      statusText = "Critical health • feed Sapito now.";
    }
  ...
  function updatePetHomePreview() {
    ...
    const personality = getPersonalityStageForLevel(state.level);
    const frogLabel = frogCount === 1 ? "Sapito Home" : "Sapitos Home";
    ui.petHomeTitle.textContent =
      `${frogLabel} • ${personality.label} • ${frogCount} pet ${frogCount === 1 ? "frog" : "frogs"}`;
    ui.petFoodButterfly.textContent = `🦋 Butterfly food: ${state.foodBalance} • 🟡 ${state.yellowButterflyBank} • 🔴 ${state.redButterflyBank}`;
    ui.petFoodWorm.textContent = `🪱 Gusanitos: ${getWormCount()}`;
    if (ui.petFoodFly) {
      ui.petFoodFly.textContent = `🪰 Flies: ${getFlyCount()} • 🏅 ${state.medalCount}`;
    }
  ...
  function renderHabitatPuzzlePreview() {
    ...
    function renderDraggablePuzzleView() {
      const targetEl = ui.habitatViewPreview;
      if (!targetEl) return;

      state.puzzleDragPositions = state.puzzleDragPositions || {};
      targetEl.innerHTML = "";
      ...
      DECOR_ORDER.forEach((id) => {
        ...
        const crop = getPuzzlePiecePreviewCrop(piece);
        const pieceEl = document.createElement("span");
        pieceEl.className = "habitat-view-piece";

        const correctX = piece.x * 100;
        const correctY = piece.y * 100;
        const width = piece.w * 100;
        const height = piece.h * 100;

        const saved = state.puzzleDragPositions[id];

        let startX = saved?.x ?? randBetween(5, 75);
        let startY = saved?.y ?? randBetween(5, 65);
        ...
        pieceEl.style.left = `${startX}%`;
        pieceEl.style.top = `${startY}%`;
        ...
        pieceEl.addEventListener("pointerdown", (event) => {
          ...
          const board = targetEl.getBoundingClientRect();
          const pieceBox = pieceEl.getBoundingClientRect();
          const offsetX = event.clientX - pieceBox.left;
          const offsetY = event.clientY - pieceBox.top;

          function movePiece(moveEvent) {
            const x = ((moveEvent.clientX - board.left - offsetX) / board.width) * 100;
            const y = ((moveEvent.clientY - board.top - offsetY) / board.height) * 100;

            const safeX = clamp(x, 0, 100 - width);
            const safeY = clamp(y, 0, 100 - height);

            pieceEl.style.left = `${safeX}%`;
            pieceEl.style.top = `${safeY}%`;

            state.puzzleDragPositions[id] = {
              x: safeX,
              y: safeY,
              placed: false
            };
          ...
          function stopMove() {
            ...
            const savedPos = state.puzzleDragPositions[id];
            if (savedPos) {
              const closeEnough =
                Math.abs(savedPos.x - correctX) < 5 &&
                Math.abs(savedPos.y - correctY) < 5;
              ...
              if (closeEnough) {
                ...
                pieceEl.style.left = `${correctX}%`;
                pieceEl.style.top = `${correctY}%`;
                pieceEl.classList.add("placed");
              }
            }

            pieceEl.removeEventListener("pointermove", movePiece);
            pieceEl.removeEventListener("pointerup", stopMove);
            pieceEl.removeEventListener("pointercancel", stopMove);
          }

          pieceEl.addEventListener("pointermove", movePiece);
          pieceEl.addEventListener("pointerup", stopMove);
          pieceEl.addEventListener("pointercancel", stopMove);
        });
  ...
  function getHomeSceneMetrics() {
    ...
    const width = Math.max(320, ...widthCandidates);
    const height = Math.max(420, ...heightCandidates);
  ...
  function ensureHomeLayerLayout(bounds = null) {
    ...
    const width = Math.max(320, Math.round(metrics.width));
    const height = Math.max(420, Math.round(metrics.height));
  ...
  function jumpHomeSapito(fromClientX, fromClientY) {
    ...
    // Restart the jump animation on repeated taps.
    void ui.homeSapito.offsetWidth;
    ui.homeSapito.classList.add("tap-jump");

    const jumpDuration = state.reduceMotion ? 220 : 430;
    const fromDx = sourceX - targetX;
    const fromDy = sourceY - targetY;
    const jumpHeight = state.reduceMotion ? -10 : -Math.min(44, Math.max(18, Math.hypot(fromDx, fromDy) * 0.22));
    if (typeof ui.homeSapito.animate === "function") {
      homeJumpAnimation = ui.homeSapito.animate(
        [
          ...
          { transform: `translate(${fromDx * 0.45}px, ${fromDy * 0.45 + jumpHeight}px) scaleX(${dir})`, offset: 0.42 },
          { transform: `translate(${fromDx * 0.12}px, ${fromDy * 0.12}px) scaleX(${dir})`, offset: 0.76 },
          { transform: `translateY(0px) scaleX(${dir})` }
        ],
        {
  ...
  async function buyCoinPack(packId) {
    ...
    showToast(
      state.language === "es"
        ? "Pago real listo solo dentro de Discord. En local no deposita coins."
        : "Real payment is ready only inside Discord. Local mode does not deposit coins."
    );
  }
  ...
  function updateSkinShopInfo() {
    ...
    if (!nextSkin) {
      ui.skinShopInfo.textContent = state.language === "es"
        ? "Todas las skins desbloqueadas. Coleccion completa."
        : "All skins unlocked. Great collection.";
      ui.buySkinBtn.textContent = state.language === "es" ? "Todas las skins compradas" : "All Skins Owned";
      ui.buySkinBtn.disabled = true;
      if (ui.catalogThemeInfo) {
        ui.catalogThemeInfo.textContent = state.language === "es"
          ? "Todas las skins tematicas desbloqueadas."
          : "All theme skins unlocked.";
      }
  ...
  function updateHabitatPanel() {
    ...
    const healthBand = getFoodHealthBand(daysLeft);
    const daysShort = daysLeft >= 10 ? Math.floor(daysLeft) : Number(daysLeft.toFixed(1));
    ...
    if (state.language === "es") {
      ui.frogStatusLine.textContent =
        `Nivel ${state.level} • Etapa ${personality.stageId}: ${personality.label} • ${frogCount} ${frogLabel} activos. ${progressText}`;
      ui.foodStatusLine.textContent =
        `Banco de comida: ${state.foodBalance} mariposas. Consumo diario: ${dailyNeed}.`;
    } else {
      ui.frogStatusLine.textContent =
        `Level ${state.level} • Stage ${personality.stageId}: ${personality.label} • ${frogCount} ${frogLabel} active. ${progressText}`;
      ui.foodStatusLine.textContent =
    ...
    if (ui.rewardStatusLine) {
      ...
      if (state.language === "es") {
        ui.rewardStatusLine.textContent =
          `Premio nivel ${state.level} (${rules.game.game}/8): ${levelPrizeText}. ` +
          (rules.stageEndsOnWin
            ? `Tambien desbloqueas bono de etapa: ${stagePrizeText}.`
            : `Bono al completar 8 juegos de etapa: ${stagePrizeText}.`) +
          ` La tienda usa Sapito Coins.`;
      } else {
        ui.rewardStatusLine.textContent =
          `Level ${state.level} reward (${rules.game.game}/8): ${levelPrizeText}. ` +
          (rules.stageEndsOnWin
            ? `You also unlock stage bonus: ${stagePrizeText}.`
            : `Stage bonus at 8/8: ${stagePrizeText}.`) +
    ...
    if (ui.foodHealthLine) {
      ...
      if (state.language === "es") {
        if (healthBand === "safe") {
          ui.foodHealthLine.textContent = `Comida estable: ~${daysShort} dias.`;
        } else if (healthBand === "low") {
          ui.foodHealthLine.textContent = `Comida baja: ~${daysShort} dias.`;
        } else {
          ui.foodHealthLine.textContent = `Critico: menos de 1 dia de comida.`;
        }
      } else if (healthBand === "safe") {
        ui.foodHealthLine.textContent = `Food status: Safe (~${daysShort} days left).`;
      } else if (healthBand === "low") {
        ui.foodHealthLine.textContent = `Food status: Low (~${daysShort} days left).`;
      } else {
        ui.foodHealthLine.textContent = "Food status: Critical (under 1 day left).";
      }
    }

    updateHomeHealthMeter(daysLeft, healthBand);
  ...
  function applyDailyFeedingDecay() {
    ...
    state.dailyFeedMessage = eaten > 0
      ? `Froggies ate ${eaten} food while you were away.`
      : "Froggies are hungry. Play to feed them.";
    updateHud();
    updateHabitatPanel();
    saveSettings();
  }
  ...
  function getTargetScore() {
    ...
    const multiplier = state.playType === "team" ? 1.6 : 1;
    return Math.max(1, Math.round(baseTarget * multiplier));
  }

  function getHelperMaxScoreContribution() {
    const target = getTargetScore();
    const modeRatio = state.mode === "easy" ? 0.26 : 0.2;
    const teamBonus = state.playType === "team" ? 0.08 : 0;
  ...
  function getHelperCatchChance() {
    let chance = state.mode === "easy" ? 0.56 : 0.46;
  ...
  function getFrogBiteCooldownMs() {
    return state.mode === "easy" ? 620 : 760;
  }
  ...
  function updatePlayTypeHint() {
    ...
    const target = getTargetScore();
    if (state.language === "es") {
      ui.playTypeHint.textContent = config.id === "team"
        ? `Modo equipo: 2 jugadores pueden jugar juntos. Meta actual: ${target} en 60 segundos.`
        : `Modo solo: 1 jugador. Meta actual: ${target} en 60 segundos.`;
    } else {
      ui.playTypeHint.textContent = config.id === "team"
        ? `Team mode: 2 players can click together. Current target: ${target} in 60 seconds.`
        : `Solo mode: 1 player. Current target: ${target} in 60 seconds.`;
    }
  }
  ...
  function updateFrogs(dt) {
    ...
    for (let i = 0; i < state.frogs.length; i += 1) {
      ...
      const target = goldenTarget || (helperAssistEnabled ? getNearestButterfly(centerX, centerY, { excludeGolden: true }) : null);
      ...
      if (target) {
          const prediction = target.kind === "golden" ? 0.15 : 0.08;
          const mistake = target.kind === "golden" ? 18 : 8;

          dx = target.x + target.size / 2 + (target.vx || 0) * prediction + randBetween(-mistake, mistake) - centerX;
          dy = target.y + target.size / 2 + (target.vy || 0) * prediction + randBetween(-mistake, mistake) - centerY;
  ...
  function spawnButterfly(options = {}) {
    ...
    const baseSpeed = state.mode === "easy" ? 68 : 92;
    const levelSpeed = (state.level - 1) * 8;
    const motionScale = state.reduceMotion ? 0.72 : 1;
    const goldenScale = isGolden ? 0.64 : 1;
    const speed = (baseSpeed + levelSpeed + randBetween(-8, 16)) * motionScale * goldenScale;
  ...
  function getSpawnInterval() {
    ...
    const modeBase = state.mode === "easy" ? 650 : 520;
    const levelPush = (state.level - 1) * 52;
    const timePush = elapsed * 6.2;
    const motionScale = state.reduceMotion ? 1.22 : 1;
    const playTypeScale = state.playType === "team" ? 0.84 : 1;

    return Math.max(120, (modeBase - levelPush - timePush) * motionScale * playTypeScale);
  }
  ...
  function tryHelperCatchGoldenButterfly(b, frog, nowMs) {
    ...
    jumpGoldenButterflyAway(
      ...
      { jump: randBetween(96, 154), sidePush: 120, speedScale: 0.42 }
    );
  ...
  function checkCatchCollisions() {
    ...
    const easyBonus = state.mode === "easy" ? 16 : 6;
  ...
  function getPlayerSpeed() {
    const modeSpeed = state.mode === "easy" ? 240 : 285;
  ...
  function startGameFromHome() {
    state.mode = ui.modeSelect.value === "normal" ? "normal" : "easy";
  ...
  function finishGame() {
    ...
    ui.resultTitle.textContent = win ? (teamMode ? "Team Win!" : "You Win!") : "Time Up";
    ui.resultMessage.textContent = win
      ? (teamMode
        ? `Great teamwork. You caught ${state.score} and beat the team goal. Reward: ${levelPrizeText}.`
        : `Amazing. You caught ${state.score} and passed the level. Reward: ${levelPrizeText}.`)
      : (teamMode
    ...
    ui.resultStats.textContent =
      `Best Score: ${state.bestScore} | Next Level: ${state.level} | ${playType.label} | Personality: ${personalityAfter.label} | Food: ${state.foodBalance} | Coins: ${state.coinBalance} | Worms: ${state.wormBank} | Flies: ${state.flyBank} | Medals: ${state.medalCount} | Helper: ${state.helperScore}/${helperCap}`;
    ...
    if (win) {
      ...
      ui.restartBtn.innerHTML = `<span class="frog-icon" aria-hidden="true">🐸</span><span class="frog-text">Try Again</span>`;
      ui.restartBtn.classList.add("retry-frog");
      ui.quitBtn.innerHTML = `<span class="frog-icon" aria-hidden="true">🐸</span><span class="frog-text">Quit</span>`;
  ...
  function setupResponsiveViewport() {
    // Optimize viewport for mobile, tablet, and Discord
    // Prevent unintended zoom on double-tap
    let lastTouchTime = 0;
    ...
    // Disable text selection on game elements for better UX on mobile
    ...
    // Optimize for notch/safe area on modern devices
    ...
    document.addEventListener(
      "touchmove",
      (e) => {
        // Allow touch on scrollable elements
        if (
          !e.target.closest(".shop-sheet") &&
          !e.target.closest(".settings-sheet") &&
          !e.target.closest("[role='scrollable']")
        ) {
          e.preventDefault();
        }
      },
    ...
    // Fix viewport for Discord embedded context
    if (isDiscordActivityContext()) {
      // Ensure app fits within Discord's embedded context
  ...
  function boot() {
    ...
    document.body.classList.toggle("discord-activity", isDiscordActivityContext());
    
    // Setup responsive viewport handling for mobile, tablet, and Discord
