(() => {
  ...
  const PUZZLE_ROWS = 4;
  const PUZZLE_COLS = 5;
  const HABITAT_TARGET_IMAGE_ASPECT = 1353 / 1163;
  ...
  function createHabitatPuzzleGuideElement(bounds) {
    if (!ui.homeDecorLayer || !bounds) return null;
    const completedPieces = getCompletedPuzzlePieceCount();
    if (completedPieces <= 0) return null;

    const el = document.createElement("div");
    el.className = "pond-puzzle-guide";
    el.style.left = `${bounds.left}px`;
    el.style.top = `${bounds.top}px`;
    el.style.width = `${bounds.width}px`;
    el.style.height = `${bounds.height}px`;
    el.style.backgroundImage = `url("${HABITAT_TARGET_IMAGE}")`;
    el.style.zIndex = "0";
    ui.homeDecorLayer.appendChild(el);
    return el;
  }
  ...
  function getHabitatPuzzleArtBounds(sceneBounds) {
    ...
    const puzzleAspect = HABITAT_TARGET_IMAGE_ASPECT;
    ...
    if (document.body.classList.contains("discord-activity") && sceneAspect > puzzleAspect * 1.22) {
      const edge = clamp(sceneWidth * 0.05, 44, 96);
      const topReserve = clamp(sceneHeight * 0.18, 112, 168);
      const bottomReserve = clamp(sceneHeight * 0.21, 124, 174);
  ...
  function renderHomeDecor() {
    ...
    const guideEl = createHabitatPuzzleGuideElement(habitatBounds);
    if (guideEl) {
      state.homeDecorElements.push(guideEl);
    }
    DECOR_ORDER.forEach((id) => {
      const count = getDecorCount(id);
      if (count <= 0) return;
