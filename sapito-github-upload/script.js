(() => {
  ...
  function getHabitatPuzzleArtBounds(sceneBounds) {
    ...
    if (document.body.classList.contains("discord-activity") && sceneAspect > puzzleAspect * 1.22) {
      const edge = clamp(sceneWidth * 0.03, 24, 72);
      const topReserve = clamp(sceneHeight * 0.06, 46, 80);
      const bottomReserve = clamp(sceneHeight * 0.12, 84, 128);
      const availableHeight = Math.max(1, sceneHeight - topReserve - bottomReserve);
      const maxPuzzleHeight = sceneHeight * 1.12;
      let width = Math.max(1, sceneWidth - (edge * 2));
      let height = width / puzzleAspect;

      if (height > maxPuzzleHeight) {
        height = maxPuzzleHeight;
  ...
  async function buyCoinPack(packId) {
    ...
    if (discordStoreUrl) {
      ...
      window.open(discordStoreUrl, "_blank", "noopener,noreferrer");
      return;
    }
