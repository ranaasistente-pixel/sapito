...
const DISCORD_CLIENT_ID = process.env.DISCORD_CLIENT_ID || DISCORD_APPLICATION_ID;
...
async function handleApi(req, res, url) {
  ...
  if (url.pathname === "/api/discord/config" && req.method === "GET") {
    sendJson(res, 200, {
      ...
      discordStoreAppId: DISCORD_APPLICATION_ID || DISCORD_CLIENT_ID,
      discordSkuIds: Object.fromEntries(Object.entries(COIN_PACKS).map(([packId, pack]) => [
        packId,
        pack.discordSkuId
      ]))
    });
